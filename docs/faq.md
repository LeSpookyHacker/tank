...
## Getting started with no documents

### I don't have any documents yet — can I still use Tank?

Yes. This is the default experience. Tank's **intake interview** (`/intake`) asks you
20 questions about your company — what it builds, who the customers are, what cloud
providers you use, what security tools (if any) exist — and produces a Day-1 Brief and
a skeleton entity graph before you upload a single file.

Start at `/intake` immediately after setup. See [first-run.md](first-run.md) for the
full walkthrough.

### What is the intake interview?

A one-question-at-a-time conversational flow that takes about 15 minutes. You answer
from memory; guesses and estimates are fine. When you finish, Tank:

- Creates **entity stubs** for every named service, team, tool, cloud provider, and
  data type you mentioned.
- Generates a **Day-1 Brief** with: what Tank now knows about your company, the top 3
  probable risk areas based on your profile, who to meet in week 1, and — most
  importantly — an explicit list of what Tank _couldn't_ determine from the interview
  alone (your investigation list for weeks 1–2).

### What are entity stubs?

Entities created from the intake interview, org discovery wizard, or chat (before any
document confirms them) are called **stubs**. They carry `confidence: 0.3` and are
visually distinct in the entity browser (dashed border, muted color). As you ingest
documents that mention the same service or team, confidence rises and the stub becomes
a full entity.

### What is "discovery mode" in chat?

When your knowledge base has fewer than 10 chunks, the chat loop automatically switches
to discovery mode. In this mode:

- Tank skips KB retrieval (there's nothing to retrieve) and switches to a
  discovery-focused system prompt.
- It acknowledges what it doesn't know directly: "I don't have any information about
  that service yet."
- It asks one clarifying question per response to help you fill in the picture.
- When you mention a new service, tool, or team name, it offers a confirmation chip at
  the end of the response — click to add it as an entity stub.

A "Discovery mode — KB building" badge appears in the chat header. It disappears once
your KB has more than 10 chunks.

### How does the 90-day plan work?

The 90-Day Plan (`/plan`) is generated by clicking "Generate plan." Tank uses your
intake answers, entity graph, and compliance targets to produce a week-by-week task
list for weeks 1–13. Each task has: a title, a description, a "why it matters" line,
and a specific testable done condition (e.g., "Threat model completed for Payments API
with at least 5 STRIDE findings recorded").

Tasks are tagged by source: `intake` (from your interview answers), `kb_state` (from
the entity graph), `compliance` (from your compliance targets), or `milestone`
(standard first-hire checkpoints). You check them off as you go. Current week's tasks
appear on the home dashboard.

You can regenerate the plan at any time — a monthly nudge prompts you when it's 30
days stale.

### What security policies does Tank generate?

Five foundational first-draft policies, generated from your KB context and org profile:

| Policy | Kind |
|--------|------|
| Acceptable Use Policy | `acceptable_use` |
| Incident Response Policy | `incident_response` |
| Secure Development Lifecycle (SDL) Policy | `secure_sdl` |
| Vulnerability Management Policy | `vulnerability_management` |
| Data Classification Policy | `data_classification` |

These are **starting points you own and edit**, not generic templates. Tank uses your
company's actual service names, cloud providers, authentication system, and data types
from the entity graph — so the IR Policy says "for incidents affecting the Payments
service, notify the Platform team" rather than "[TEAM NAME]."

Each policy is versioned and stored as a living artifact (same pattern as threat
models). Status lifecycle: draft → reviewed → approved.

See [features/build.md](features/build.md) for the full reference.

### What is the Prioritization Engine?

The Prioritization Engine takes your risk register, entity graph, security stack audit,
compliance targets, and open vulnerabilities, then produces a **concrete quarterly
top-5 action plan** — not a sorted list, an opinion.

For each of the 5 items it gives you: what the risk is and which service it affects,
*why* it's in the top 5 (one plain-language sentence, not a CVSS score), what "done"
looks like (specific and testable), estimated effort (days/weeks/months), and the
service owner from the entity graph.

Access it from the Security Program Dashboard (`/security-program`) via "Generate
priorities." The output is cached with a "Generated [date]" label and a Regenerate
button. A monthly nudge fires when the output is 30+ days old.

See [features/build.md](features/build.md) for the full reference.

### What is the Compliance Framework Wizard?

An 8-question decision tree at `/compliance/wizard` that recommends the right
compliance framework for your company based on: industry, customer type, whether you
handle payment or health data, customer contract requirements, investor pressure,
government contracts, and your realistic timeline.

Output: a ranked recommendation of the top 2 frameworks with business-language
rationale, an honest effort estimate (engineer-months), common blockers, and a gap
analysis against your current KB state. The recommendation is stored as a
`design_choice` entry in your decisions log.
